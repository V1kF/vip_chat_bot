    ...
    while True:
        try:
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute(f"SELECT id,vip_chat,notification FROM {TABLE_NAME} WHERE vip_chat!=0;")  # выполняет запрос к базе данных
                ...
                if cursor.fetchall() != ():  # если человек/люди с подпиской найден/ы
                    ...
                    for subscriber in SUBSCRIBERS_CHANNEL:
                        ...
                        if subscriber['vip_chat']<=time_now:#если время подписки больше или равно времени сейчас запрещаем доступ к каналу
                            ...
                            ban = json.loads(ban_str)  # переделываем в словарь
        ...
        except Exception as ex:  # объект исключений Exception будет записан в переменную ex
    ...
    while True:
        ...
        try:
            ...
            if receiving_messages['ok'] and receiving_messages['result']!=[]:
                ...
                with connection.cursor() as cursor:
                    cursor.execute(f"SELECT id FROM {TABLE_NAME} WHERE vip_chat!=0;")  # выполняет запрос к базе данных
                    connection.commit()  # подтверждает все незавершённые транзакции
                    ...
                    if user.get('message') and user['message'].get('new_chat_members') and\
                        ...
                        for honest_person in honest_persons:
                            ...
                            if honest_person['id']==user['message']['new_chat_members']['id']:#если человек один из приобрётших людей подписку
                                ...
                                ban=False#ставим фолз,что значит что человек не халявщик

                        if  ban==True:#если человек-халявщик,баним его

                            ban_str = requests.get(f'{URL}{TOKEN}/banChatMember?chat_id={CHANNEL_CHAT_ID}&user_id={user["message"]["new_chat_members"]["id"]}')
                            ban_str = ban_str.text
                            ban_pers = json.loads(ban_str)  # переделываем в словарь
            ...
            if update_id<event['update_id']:
                ...
                if  event.get('message'):
                    ...
                    with connection.cursor() as cursor:
                        ...
                        cursor.execute(f"SELECT id FROM {TABLE_NAME} WHERE id= {id_pers};")  # выполняет запрос к базе данных
                        connection.commit()  # подтверждает все незавершённые транзакции
                ...
                if 'callback_query' in event:  # Если нажата кнопка
                    ...
                    if data=='ref_link':
                        ...
                        with connection.cursor() as cursor:
                            ...
                            cursor.execute(f"SELECT promo_pers FROM {TABLE_NAME} WHERE id= {id_pers};")  # выполняет запрос к базе данных
                            connection.commit()  # подтверждает все незавершённые транзакции
                        ...
                        #Ваша ссылка - приглашение: https://t.me/crypt_for_a_secret_bot?start={id_pers}
                        answer_str = requests.get(f'{URL}{TOKEN}/answerCallbackQuery?callback_query_id={event["callback_query"]["id"]}')
                        answer_str = answer_str.text
                        answer = json.loads(answer_str)  # переделываем в словарь
                    ...
                    if data == 'subscription':
                        ...
                        with connection.cursor() as cursor_1:
                            ...
                            # берём номер счёта
                            cursor_1.execute(f"SELECT billid FROM {TABLE_NAME} WHERE id= {id_pers};")  # выполняет запрос к базе данных
                            connection.commit()  # подтверждает все незавершённые транзакции
                        ...
                        with connection.cursor() as cursor_2:
                            ...
                            cursor_2.execute(f"SELECT vip_chat FROM {TABLE_NAME} WHERE id= {id_pers};")  # выполняет запрос к базе данных
                            connection.commit()  # подтверждает все незавершённые транзакции
                    ...
                    if data == 'check':
                        ...
                        with connection.cursor() as cursor:
                            ...
                            cursor.execute(f"SELECT vip_chat FROM {TABLE_NAME} WHERE id= {id_pers} AND vip_chat!=0;")  # Если человек нажимает кнопку не первый раз уже оплатив чтобы ему была доступна ссылка
                            connection.commit()  # подтверждает все незавершённые транзакции
                            ...
                            # берём номер счёта
                            cursor.execute(f"SELECT billid FROM {TABLE_NAME} WHERE id= {id_pers};")  # выполняет запрос к базе данных
                            connection.commit()  # подтверждает все незавершённые транзакции
                        ...
                        resp=json.loads(resp)  # переделываем в словарь
                        ...
                        if resp["status"]["value"]=="PAID":
                            ...
                            if not infoChatMember['result'].get('until_date'):#если человек заблокирован
                                ...
                                unban = json.loads(unban_str)  # переделываем в словарь
                            ...
                            days_30=2592000 #количсетво секунд в 30 днях
                    ...
                    if data == 'subscription_date':
                        ...
                        with connection.cursor() as cursor:
                            ...
                            cursor.execute(f"SELECT vip_chat FROM {TABLE_NAME} WHERE id= {id_pers};")  # выполняет запрос к базе данных
                            connection.commit()  # подтверждает все незавершённые транзакции
                    ...
                    if data == 'payouts':#кнопка по которой админы и владелец сможет делать выплаты участникам канала
                ...
                if 'message' in event:#Сообщение любого вида
                    ...
                    if  event['message'].get('text'):
                        ...
                        if re.fullmatch(r"\/start (.+)", text):
                            ...
                            with connection.cursor() as cursor:
                                # добавляем в графу реф ссылки друга(0-отсутствует)
